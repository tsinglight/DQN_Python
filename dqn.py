...
class Agent():
    ...
    def build_network(self):
        model = Sequential()
        model.add(Convolution2D(filters=32, kernel_size=8,strides= (4, 4),padding='same', activation='relu', input_shape=(STATE_LENGTH, FRAME_WIDTH, FRAME_HEIGHT)))
        model.add(Convolution2D(filters=64, kernel_size=4,strides=(2, 2),padding='same', activation='relu'))
        model.add(Convolution2D(filters=64, kernel_size=3,strides=(1, 1),padding='same', activation='relu'))
        model.add(Flatten())
